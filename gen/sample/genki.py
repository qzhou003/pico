...
parser.add_argument('src', help='Ground truth file')
parser.add_argument('--plot', action='store_true', help='Show dumped images')
parser.add_argument('--out', default='images.dump', help='Output data file')
...
def export(im, r, c, s):
    ...
    # crop
    r0 = max(int(r - 0.75 * s), 0)
    r1 = min(r + 0.75 * s, nrows)
    c0 = max(int(c - 0.75 * s), 0)
    c1 = min(c + 0.75 * s, ncols)
    ...
    for i in range(0, nrands):
        ...
        if args.plot:
            ...
            matplotlib.pyplot.plot([ctmp - stmp / 2, ctmp + stmp / 2], [rtmp - stmp / 2, rtmp - stmp / 2], 'b',
                                   linewidth=3)
            matplotlib.pyplot.plot([ctmp + stmp / 2, ctmp + stmp / 2], [rtmp - stmp / 2, rtmp + stmp / 2], 'b',
                                   linewidth=3)
            matplotlib.pyplot.plot([ctmp + stmp / 2, ctmp - stmp / 2], [rtmp + stmp / 2, rtmp + stmp / 2], 'b',
...
rs_cs_ss = [(float(line.split()[1]), float(line.split()[0]), float(line.split()[2]))
            for line in open(src + '/Subsets/GENKI-SZSL/GENKI-SZSL_labels.txt', 'r').readlines()]
